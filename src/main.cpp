...
cv::Rect get_rect(std::vector<sl::uint2> box) {
    return cv::Rect(round(box[0][0]), round(box[0][1]), round(box[2][0] - box[0][0]), round(box[2][1] - box[0][1]));
}
...
int main(int argc, char** argv) {
    ...
    while (viewer.isAvailable()) {
        if (zed.grab() == sl::ERROR_CODE::SUCCESS) {
            ...
            // str(round(abs(obj.position[2])
            //cout << round(abs(objects.object_list.at(2).position[2]) * 10.0) / 10.0 << endl;
            // GL Viewer
